...
namespace Weilog.Core.Plugins
{
    public class PluginDescriptor : IComparable<PluginDescriptor>
    {
        ...
        {
            object instance;
            if (!UowKernelModule.Current.ContainerManager.TryResolve(PluginType, null, out instance))
            {
                //    //not resolved
                instance = UowKernelModule.Current.ContainerManager.ResolveUnregistered(PluginType);
            }
